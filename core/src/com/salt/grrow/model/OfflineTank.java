...
public class OfflineTank extends Tank{
     ...
     public void evaluateCreatures(){
    	 ...
    	 }else UglyOneThatFucks = itr.next();
    	 ...
    	 this.Creatures.add(this.lastGeneration.first());
    	 this.Creatures.add(this.lastGeneration.get(1));
     }
	...
	public void newGeneration() throws IOException {
     ...
     public void creatureIteration(){
    	 for(Creature c: this.Creatures){
			...
			if(hasEaten) {
				this.CheckPoints.add(new CheckPoint(this.nextIntWithNegatives(650), this.nextIntWithNegatives(350), 10));
			}
...
//			Fitness
			if(hasEaten){
				c.health+= 500+c.itEating;
				c.itEating += 100;
				c.itWithOutEating = 0;
			}
			else{
				c.itWithOutEating++;
				if(c.Brain.lrdist.getValue() < previousDist){
					c.health+=50;
					return;
				}
				else{ 
					c.itWithOutEating++;
					if(c.itWithOutEating > 200) c.itWithOutEating = 200;
					c.health -= c.itWithOutEating;
					if(tempRot==previousRot){
						c.health+=2;
					}
					if(c.canEat()){
						c.health-=1000;
					}
					if(c.getCreaturePosition().equals(previousPos)) c.health-=1000;
				}
			}
    	 }
     }
    ...
    public void saveLastGeneration() throws IOException{
    	...
    	for(Creature c : this.lastGeneration){
    		c.StoreDNA(this.getGeneration());
    	}
    }
